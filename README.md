...
- Folder URL enumeration — **deferred** (see follow-up task in sprint board)
...
| `extract_links`      | File URL → JSON `{ kind: "file", files: [FileLink] }` with `EncryptionInfo`. Folder URL currently returns an error (deferred). |
